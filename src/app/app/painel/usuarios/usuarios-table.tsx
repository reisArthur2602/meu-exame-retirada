...
import { ROLE_CONFIG, STATUS_CONFIG, getAvatarColor, getInitials } from '@/lib/usuarios';
import { useMutation } from '@tanstack/react-query';
import { useRouter, useSearchParams } from 'next/navigation';
import { useEffect, useRef, useState } from 'react';
import { toast } from 'sonner';
import { type UsuarioRow, type GetUsuariosMeta } from './actions/get-usuarios';
...
export default function UsuariosTable({ data, meta }: UsuariosTableProps) {
    ...
    function openEdit(user: UsuarioRow) { setEditingUser(user); setModalOpen(true); }
    ...
    const toggleMutation = useMutation({
        ...
        onSuccess: () => { toast.success('Status atualizado.'); router.refresh(); },
        onError: (e: Error) => toast.error('Erro', { description: e.message }),
    });

    const deleteMutation = useMutation({
        mutationFn: (id: string) => deleteUsuario(id),
        onSuccess: () => { toast.success('Usuário excluído.'); setDeletingId(null); router.refresh(); },
        onError: (e: Error) => toast.error('Erro', { description: e.message }),
    });
    ...
    return (
        <>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                <StatCard label="Total de usuários" value={String(meta.total)}        icon={STAT_ICONS.users}  />
                <StatCard label="Usuários ativos"   value={String(meta.totalAtivos)}  icon={STAT_ICONS.check}  />
                <StatCard label="Admins"            value={String(meta.totalAdmins)}  icon={STAT_ICONS.shield} />
                <StatCard label="Inativos"          value={String(meta.totalInativos)} icon={STAT_ICONS.pause} />
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center gap-3">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 sm:ml-auto">
                        ...
                        <select
                            value={roleFilter}
                            onChange={(e) => updateParams({ role: e.target.value !== 'all' ? e.target.value : null, page: '1' })}
                                        ...
                                        <td className="px-5 py-3.5">
                                            {deletingId === user.id ? (
                                                ...
                                                <div className="flex items-center gap-1 justify-end">
                                                    <ActionButton title="Editar usuário" onClick={() => openEdit(user)}>
                                                    ...
                                                    <ActionButton title="Excluir usuário" onClick={() => setDeletingId(user.id)} danger>
                                                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
function PaginationButton({ children, onClick, disabled, ...rest }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
    return (
        <button
            onClick={onClick}
            disabled={disabled}
