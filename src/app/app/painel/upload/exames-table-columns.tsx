...
import type { ExameRow } from '@/app/app/painel/upload/actions/get-exams';
import { formatCPF } from '@/lib/cpf';
import type { ColumnDef } from '@tanstack/react-table';
import { Download, Eye, PrinterIcon } from 'lucide-react';

// ─── Types ──────────────────────────────────────────────────────────────────

export interface ColumnOptions {
    onView: (row: ExameRow) => void;
    onDownload: (row: ExameRow) => void;
    onWhatsApp: (row: ExameRow) => void;
    onPrint: (row: ExameRow) => void;
}
...

        {
            ...
            cell: ({ row }) => (
                <div className="flex items-center gap-1 justify-end">
                    <ActionButton title="Ver exame" onClick={() => options.onView(row.original)}>
                    ...
                    <ActionButton
                        title={
                            row.original.telefone
                                ? `Enviar WhatsApp para ${row.original.telefone}`
                                : 'Informar telefone para enviar WhatsApp'
                        }
