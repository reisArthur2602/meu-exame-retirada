...
import { getColumns } from './exams-table-columns';
...
export default function ExamesTable({ data, meta }: ExamesTableProps) {
    ...
    function handleView(row: ExameRow) {
        router.push(
            `/exame?cpf=${encodeURIComponent(row.cpf)}&protocolo=${encodeURIComponent(row.protocolo)}`
        );
    }
                                            ...
                                            <td colSpan={columns.length} className="px-5 py-3">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="text-sm font-semibold text-green-800 shrink-0">
                                                        Telefone não cadastrado — informe para
                                                        enviar:
                                                    </span>
                                                    <input
                                                        ref={phoneInputRef}
                                                        ...
                                                        onKeyDown={(e) => {
                                                            if (e.key === 'Enter')
                                                                handlePhoneSubmit(row.original);
                                                            if (e.key === 'Escape')
                                                                setPhonePopoverId(null);
                                                        }}
            ...
            {meta.total > 0 && (
                <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between gap-4">
                    <p className="text-xs text-slate-500">
                        ...
                        de <span className="font-semibold text-slate-700">{meta.total}</span> exames
                    </p>
